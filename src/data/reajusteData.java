...
public class dReajustes {
 private final String nombreClase = "dReajustes";
...
error + e.getMessage());
 ...
 ResultSet rs = st.executeQuery("select * from reajustes where
idrea=" + id);
 if (rs.next()) {
 p = getResult(rs);
 }
...
e.getMessage());
 ...
 public cReajustes buscarIdvalOrigenNro(int idfm, int idval_origen, int
nro_valorizacion) {
 cReajustes p = null;
 try {
 conexion dbm = new conexion();
 ...
 + "inner join mes_reajuste_valorizado v2 on
v2.idmrv=v1.idmrv "
...
e.getMessage());
 ...
 public cReajustes buscarIdfmIdvaIdvalOrigenlNro(int idfm, int idval, int
idval_origen, int nro_valorizacion) {
 cReajustes p = null;
 try {
 conexion dbm = new conexion();
 Connection con = dbm.getConnection();
 ...
 + "inner join mes_reajuste_valorizado v2 on
v2.idmrv=v1.idmrv "
 + "where v2.idfm=" + idfm + " and v2.idval=" + idval + "
and v2.idval_origen=" + idval_origen + " and v2.nro_valorizacion=" +
nro_valorizacion;
206
 System.out.println("sql:" + sql);
 ResultSet rs = st.executeQuery(sql);
...
e.getMessage());
 ...
 + "INNER JOIN mes_reajuste_valorizado v1 ON
v1.idmrv=r.idmrv "
 + "WHERE v1.idfm=" + idfm + " AND r.idval<" + idval + "
AND r.`estado_obra`='ATRASADA' ";
 System.out.println("sql:" + sql);
 ResultSet rs = st.executeQuery(sql);
 if (rs.next()) {
 try {
