...
"]{eliminarIdObraDesdeValorizacion}\n" + error + e.getMessage());
 ...
 public boolean eliminarIdObraDesdeValorizacionListaPartidasActivas(int
id) {
 String error = null;
 conexion dbm = new conexion();
 try {
...
idps;
 ...
 public cValorizacionDatos buscarIdvalIdpsIdfm(int idval, int idps, int
idfm) {
 cValorizacionDatos p = null;
 try {
 conexion dbm = new conexion();
...
+ "Error :" + e.getMessage());
 ...
 public List<cValorizacionDatos> ListTodoIdvalIdovaIdfm(int idval, int
idova, int idfm) {
 List<cValorizacionDatos> lista = new
ArrayList<cValorizacionDatos>();
 try {
 conexion dbm = new conexion();
 Connection con = dbm.getConnection();
 if (con == null) {
